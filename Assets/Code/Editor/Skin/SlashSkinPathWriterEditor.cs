...
{
    [CustomEditor(typeof(SlashSkinPathWriter))]
    public class SlashSkinPathWriterEditor : UnityEditor.Editor
    {
        ...
        void OnEnable()
        {
            Target = ( SlashSkinPathWriter ) target;
        ...
        void SampleClip ( AnimationClip c, term term )
        {
            ...
            for (float t = startTime; t < endTime; t += slay.path.delta)
                sampleTimes.Add(t);
            sampleTimes.Add(endTime);

            // sample
            slay.path path = new slay.path ();
            path.key = term;
