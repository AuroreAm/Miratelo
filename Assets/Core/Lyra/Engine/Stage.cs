...
namespace Lyra
{
    public abstract class Stage : MonoBehaviour, IPixiHandler, IBlockAuthor
    {
        ...
        static int counter;
        List <sys> Pixis = new List<sys> ();
        Dictionary <int, sys> PixiIndex = new Dictionary<int, sys> ();
        Dictionary <sys, int> IndexPixi = new Dictionary<sys, int> ();
        ...
        public static int Start ( sys brick ) => o._Start (brick);
        int _Start ( sys brick )
        {
